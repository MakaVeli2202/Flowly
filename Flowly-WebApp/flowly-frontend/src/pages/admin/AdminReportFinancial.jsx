...
function FinancialReport() {
  ...
  const [datevMonth,   setDatevMonth]   = useState(format(new Date(), 'yyyy-MM'));
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                <input type="month" value={datevMonth} onChange={e => setDatevMonth(e.target.value)}
                  className="px-2 py-2 rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-color)] text-sm" />
                <button type="button" disabled={datevLoading}
                  onClick={async () => { setDatevLoading(true); try { await reportsAPI.downloadDatevExport(datevMonth); } finally { setDatevLoading(false); } }}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-color)] text-sm font-semibold hover:bg-[var(--surface-bg)] transition disabled:opacity-50">
                  <Download size={14} /> {datevLoading ? 'Exporting...' : 'DATEV Export'}
                </button>
                <button type="button"
                  onClick={() => reportsAPI.downloadIcal(startDate, endDate)}
