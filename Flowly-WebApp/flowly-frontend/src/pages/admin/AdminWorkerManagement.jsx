...
const UI_BY_LANG = {
  en: {
    ...
    conflictWarning: 'This schedule conflicts with {{count}} existing booking(s):\n\n{{details}}\n\nSave anyway?',
  ...
  ar: {
    ...
    conflictWarning: 'يتعارض هذا الجدول مع {{count}} حجز/حجوزات موجودة:\n\n{{details}}\n\nحفظ على أي حال؟',
  ...
  de: {
    ...
    conflictWarning: 'Dieser Zeitplan steht mit {{count}} bestehenden Buchung(en) in Konflikt:\n\n{{details}}\n\nTrotzdem speichern?',
...
export default function WorkerManagement() {
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      setSaving(true);
      const workingDaysStr = editWorker.workingDays.join(',');
      // Check for booking conflicts before saving
      const { hasConflicts, conflicts } = await authAPI.getScheduleConflicts(
        editWorker.id, workingDaysStr, editWorker.shiftStart, editWorker.shiftEnd
      );
      if (hasConflicts && conflicts?.length > 0) {
        const details = conflicts.map(c => `- ${c.date} ${c.timeSlot}: ${c.issue}`).join('\n');
        const msg = ui.conflictWarning
