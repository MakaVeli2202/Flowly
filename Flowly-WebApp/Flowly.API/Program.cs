...
using Flowly.API.Hubs;
using Flowly.API.Filters;
using Flowly.API.Validators;
...
builder.Services.AddScoped<Flowly.API.Services.IAuditService, Flowly.API.Services.AuditService>();
builder.Services.AddScoped<Flowly.API.Services.ICredentialVerifier, Flowly.API.Services.CredentialVerifier>();
builder.Services.AddSingleton<Flowly.API.Services.CouponRateLimiter>();
...
builder.Services.AddScoped<Flowly.API.Platform.Tenancy.TenantContext>();
// Platform: MediatR + domain event service
builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(Program).Assembly));
builder.Services.AddScoped<Flowly.API.Platform.AuditEvents.IDomainEventService, Flowly.API.Platform.AuditEvents.DomainEventService>();
builder.Services.AddScoped<Flowly.API.Modules.Booking.IBookingService, Flowly.API.Modules.Booking.BookingService>();
builder.Services.AddScoped<Flowly.API.Modules.Organization.IOrganizationService, Flowly.API.Modules.Organization.OrganizationService>();
builder.Services.AddScoped<Flowly.API.Modules.Billing.IBillingService, Flowly.API.Modules.Billing.BillingService>();
builder.Services.AddScoped<Flowly.API.Modules.FeatureFlags.IFeatureFlagService, Flowly.API.Modules.FeatureFlags.FeatureFlagService>();
builder.Services.AddScoped<Flowly.API.Modules.Permissions.IPermissionsService, Flowly.API.Modules.Permissions.PermissionsService>();
builder.Services.AddScoped<Flowly.API.Modules.Resources.IResourceService, Flowly.API.Modules.Resources.ResourceService>();
builder.Services.AddScoped<Flowly.API.Modules.IndustryTemplates.IIndustryTemplateService, Flowly.API.Modules.IndustryTemplates.IndustryTemplateService>();
builder.Services.AddScoped<Flowly.API.Modules.CustomFields.ICustomFieldService, Flowly.API.Modules.CustomFields.CustomFieldService>();
builder.Services.AddScoped<Flowly.API.Modules.ClientAssets.IClientAssetService, Flowly.API.Modules.ClientAssets.ClientAssetService>();
builder.Services.AddScoped<Flowly.API.Modules.Automation.IAutomationRuleService, Flowly.API.Modules.Automation.AutomationRuleService>();
builder.Services.AddScoped<Flowly.API.Modules.Auth.IAuthService, Flowly.API.Modules.Auth.AuthService>();
builder.Services.AddScoped<Flowly.API.Modules.Staff.IStaffService, Flowly.API.Modules.Staff.StaffService>();
builder.Services.AddScoped<Flowly.API.Modules.Offers.IOfferService, Flowly.API.Modules.Offers.OfferService>();
builder.Services.AddScoped<Flowly.API.Modules.CRM.ICrmService, Flowly.API.Modules.CRM.CrmService>();
builder.Services.AddScoped<Flowly.API.Modules.Packages.IPackageService, Flowly.API.Modules.Packages.PackageService>();
builder.Services.AddScoped<Flowly.API.Modules.Settings.ISettingsService, Flowly.API.Modules.Settings.SettingsService>();
builder.Services.AddScoped<Flowly.API.Modules.SubscriptionBookings.ISubscriptionBookingService, Flowly.API.Modules.SubscriptionBookings.SubscriptionBookingService>();
builder.Services.AddScoped<Flowly.API.Modules.Reports.IReportsService, Flowly.API.Modules.Reports.ReportsService>();
builder.Services.AddScoped<Flowly.API.Modules.Plans.IPlansService, Flowly.API.Modules.Plans.PlansService>();
builder.Services.AddScoped<Flowly.API.Modules.Services.IServicesService, Flowly.API.Modules.Services.ServicesService>();
builder.Services.AddScoped<Flowly.API.Modules.RecurringBookings.IRecurringBookingService, Flowly.API.Modules.RecurringBookings.RecurringBookingService>();
builder.Services.AddScoped<Flowly.API.Modules.Waitlist.IWaitlistService, Flowly.API.Modules.Waitlist.WaitlistService>();
builder.Services.AddScoped<Flowly.API.Modules.AI.IAIService, Flowly.API.Modules.AI.AIService>();
builder.Services.AddSingleton<Flowly.API.Modules.Webhooks.IWebhookService, Flowly.API.Modules.Webhooks.WebhookService>();
builder.Services.AddHttpClient("Webhook").ConfigureHttpClient(c => c.Timeout = TimeSpan.FromSeconds(12));
...
if (smsEnabled && !string.IsNullOrWhiteSpace(builder.Configuration["Infobip:ApiKey"]))
    builder.Services.AddScoped<Flowly.API.Platform.Messaging.ISmsService, Flowly.API.Platform.Messaging.InfobipSmsService>();
else
    builder.Services.AddScoped<Flowly.API.Platform.Messaging.ISmsService, Flowly.API.Platform.Messaging.NullSmsService>();
builder.Services.AddScoped<Flowly.API.Platform.Messaging.BookingReminderJob>();
...
// Notification jobs: birthday/anniversary offers, review requests, booking reminders
builder.Services.AddHostedService<BirthdayOfferJob>();
builder.Services.AddHostedService<ReviewRequestPushJob>();
builder.Services.AddHostedService<PushReminderJob>();
builder.Services.AddHostedService<AIWeeklyDigestJob>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    if (hoursRow != null && !string.IsNullOrWhiteSpace(hoursRow.Value))
    {
        try
        {
            ...
            if (hours != null)
            {
                Flowly.API.Modules.Booking.BookingService.SetBusinessHoursFromSettings(hours);
...
Flowly.API.Modules.Booking.BookingService.ApplyConfiguredTimeZone(configuredTz);
...
app.UseStaticFiles();
app.UseMiddleware<Flowly.API.Middleware.CorrelationIdMiddleware>();
app.UseIpRateLimiting();
app.UseCors("AllowFrontend");
app.UseAuthentication();
app.UseAuthorization();
// Platform: resolve tenant context after auth so JWT claims are available
app.UseMiddleware<Flowly.API.Platform.Tenancy.TenantMiddleware>();
app.UseMiddleware<Flowly.API.Platform.Tenancy.PlanGuardMiddleware>();
app.MapControllers();
...
WHERE bi.""PackageId"" = p.""Id""
  ...
  AND p.""EstimatedDurationMinutes"" > 0;";
        ...
        await EnsureAttendanceTableAsync(connection);
...
static async Task EnsurePageViewsTableAsync(DbConnection connection)
{
    await using var cmd = connection.CreateCommand();
    cmd.CommandText = @"
CREATE TABLE IF NOT EXISTS ""PageViews"" (
    ...
    ""Page"" character varying(255) NOT NULL DEFAULT '/',
    ""Source"" character varying(50) NOT NULL DEFAULT 'Direct',
    ""Referrer"" character varying(500) NULL,
    ""DeviceType"" character varying(20) NOT NULL DEFAULT 'Desktop',
...
CREATE INDEX IF NOT EXISTS ""IX_PageViews_SessionId"" ON ""PageViews"" (""SessionId"");
CREATE INDEX IF NOT EXISTS ""IX_PageViews_LastHeartbeat"" ON ""PageViews"" (""LastHeartbeat"");
CREATE INDEX IF NOT EXISTS ""IX_PageViews_FirstSeen"" ON ""PageViews"" (""FirstSeen"");
";
    await cmd.ExecuteNonQueryAsync();
}
...
static async Task EnsureAttendanceTableAsync(DbConnection connection)
{
    await using var cmd = connection.CreateCommand();
    cmd.CommandText = @"
CREATE TABLE IF NOT EXISTS ""AttendanceLogs"" (
    ...
    ""ClockIn"" timestamp with time zone NULL,
    ""ClockOut"" timestamp with time zone NULL,
    ""Note"" character varying(500) NULL,
    ""CreatedAt"" timestamp with time zone NOT NULL DEFAULT NOW(),
    ""UpdatedAt"" timestamp with time zone NOT NULL DEFAULT NOW(),
    CONSTRAINT ""FK_AttendanceLogs_Staff_StaffId"" FOREIGN KEY (""StaffId"") REFERENCES ""Staff""(""Id"") ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS ""IX_AttendanceLogs_StaffId"" ON ""AttendanceLogs"" (""StaffId"");
CREATE INDEX IF NOT EXISTS ""IX_AttendanceLogs_ShiftDate"" ON ""AttendanceLogs"" (""ShiftDate"");
";
    await cmd.ExecuteNonQueryAsync();
}
