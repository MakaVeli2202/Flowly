...
namespace Flowly.API.Modules.Plans
{
    public class PlansService : IPlansService
    {
        private readonly AppDbContext _context;

        public PlansService(AppDbContext context)
        {
            _context = context;
        }

        public async Task<IEnumerable<SubscriptionPlanDto>> GetPlansAsync(VehicleType? vehicleType)
        {
            ...
            return plans.Select(p => ToPlanDto(p, 0));
        }

        public async Task<IEnumerable<SubscriptionPlanDto>> GetAdminPlansAsync()
        {
            ...
            return plans.Select(p => ToPlanDto(p, p.UserSubscriptions.Count(s => s.Status == UserSubscriptionStatus.Active)));
        }

        public async Task<IEnumerable<UserSubscriptionAdminDto>> GetSubscribersAsync(UserSubscriptionStatus? status)
        {
            ...
            var subscriptions = await query.OrderByDescending(s => s.StartDate).ToListAsync();

            return subscriptions.Select(s => new UserSubscriptionAdminDto
            {
                Id              = s.Id,
                UserId          = s.UserId,
        ...
        public async Task<SubscriptionPlanDto> CreatePlanAsync(UpsertSubscriptionPlanDto dto)
        {
        ...
        public async Task<(SubscriptionPlanDto? Result, string? Error)> UpdatePlanAsync(int id, UpsertSubscriptionPlanDto dto)
        {
            ...
            if (plan == null) return (null, "Subscription plan not found.");

            ApplyPlanUpdate(plan, dto);
            await _context.SaveChangesAsync();

            return (ToPlanDto(plan, plan.UserSubscriptions.Count(s => s.Status == UserSubscriptionStatus.Active)), null);
        }

        public async Task<(string? Error, string Message)> DeletePlanAsync(int id)
        {
            var plan = await _context.SubscriptionPlans
                .Include(p => p.UserSubscriptions)
                .FirstOrDefaultAsync(p => p.Id == id);

            if (plan == null) return ("Subscription plan not found.", "");
            ...
            if (plan.UserSubscriptions.Any())
            {
                ...
                await _context.SaveChangesAsync();
                return (null, "Plan deactivated because subscriptions already reference it.");
            }

            _context.SubscriptionPlans.Remove(plan);
            await _context.SaveChangesAsync();
            return (null, "Plan deleted.");
        }

        public async Task<(CustomerSubscriptionDto? Result, string? Error)> SubscribeAsync(int planId, int? userId)
        {
            var plan = await _context.SubscriptionPlans
                .Include(p => p.Features)
                .Include(p => p.Benefits)
            ...
            if (plan == null) return (null, "Subscription plan not found or inactive.");
            ...
            return (ToCustomerSubscriptionDto(subscription), null);
        }

        public async Task<(CustomerSubscriptionDto? Result, string? Error)> GetMySubscriptionAsync(int? userId)
        {
            var subscription = await _context.UserSubscriptions
                .AsNoTracking()
                .Include(s => s.Plan).ThenInclude(p => p.Features)
                .Include(s => s.Plan).ThenInclude(p => p.Benefits)
                .Include(s => s.Plan).ThenInclude(p => p.PlanPackages).ThenInclude(pp => pp.Package)
                .Where(s => s.UserId == userId && s.Status == UserSubscriptionStatus.Active)
                .OrderByDescending(s => s.StartDate)
                .FirstOrDefaultAsync();

            if (subscription == null) return (null, "No active subscription found.");
            return (ToCustomerSubscriptionDto(subscription), null);
        }

        public async Task<(string? Error, string Message)> CancelMySubscriptionAsync(int? userId)
        {
            var subscription = await _context.UserSubscriptions
                .Where(s => s.UserId == userId && s.Status == UserSubscriptionStatus.Active)
                .OrderByDescending(s => s.StartDate)
            ...
            if (subscription == null) return ("No active subscription found.", "");

            subscription.Status    = UserSubscriptionStatus.Cancelled;
            subscription.UpdatedAt = DateTime.UtcNow;
            await _context.SaveChangesAsync();
            return (null, "Subscription cancelled.");
        }

        // ── Public static mappers ─────────────────────────────────────────────

        public static CustomerSubscriptionDto ToCustomerSubscriptionDto(UserSubscription subscription) => new()
        {
            Id              = subscription.Id,
        ...
        private static SubscriptionPlanDto ToPlanDto(SubscriptionPlan plan, int subscriberCount) => new()
        {
            Id              = plan.Id,
            Name            = plan.Name,
        ...
        private static void ApplyPlanUpdate(SubscriptionPlan plan, UpsertSubscriptionPlanDto dto)
        {
            ...
            plan.Features.Clear();
            for (var i = 0; i < dto.Features.Count; i++)
            {
                var text = dto.Features[i]?.Trim();
                if (!string.IsNullOrWhiteSpace(text))
                    plan.Features.Add(new SubscriptionPlanFeature { FeatureText = text, DisplayOrder = i });
            }

            plan.Benefits.Clear();
            for (var i = 0; i < dto.Benefits.Count; i++)
            {
                var text = dto.Benefits[i]?.Trim();
                if (!string.IsNullOrWhiteSpace(text))
                    plan.Benefits.Add(new SubscriptionPlanBenefit { BenefitText = text, DisplayOrder = i });
            }

            plan.PlanPackages.Clear();
            for (var i = 0; i < dto.PackageIds.Count; i++)
                plan.PlanPackages.Add(new SubscriptionPlanPackage { PackageId = dto.PackageIds[i], DisplayOrder = i });
        }
    }
}
