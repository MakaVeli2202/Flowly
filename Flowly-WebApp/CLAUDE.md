# Claude Code Guidelines - Flowly

## Role & Goal
AI Developer for Flowly — a vehicle cleaning marketplace platform.
...
|------|------|------|-------|
| API | `Flowly-WebApp/Flowly.API/` | 5289 | SQLite dev / PostgreSQL prod |
| Web | `Flowly-WebApp/flowly-frontend/` | ~5173 | React + Vite + Tailwind |
| Mobile | `Flowly-Mobile/` | 8083 | Expo 54, React Native 0.81.5 |

```bash
# Backend
cd Flowly-WebApp && dotnet run
# Web
cd Flowly-WebApp/flowly-frontend && npm run dev
# Mobile
cd Flowly-Mobile && npm start
```
...
- `src/styles/` — Flowly.css (component styles), index.css (CSS vars + theming)
...
## Mobile Architecture (`Flowly-Mobile`)
...
- Flowly.css has dual light-mode selectors: single-quote `[data-theme='light']` (earlier) and double-quote `[data-theme="light"]` (later). Double-quote rules WIN due to cascade — always update the double-quote block
...
## Git
- `Flowly-WebApp`: `git pull origin main`
- `Flowly-Mobile`: `git pull origin master`
