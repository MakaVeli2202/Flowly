  ...
  return (
    <View style={styles.root}>
      <View style={[styles.header, headerStyle]}>
        <Text style={[styles.title, { opacity: headerOpacity }]}>{t('admin.plans.title', 'Subscription Plans')}</Text>
      </View>
      <ScrollView style={styles.scroll} onScroll={handleScroll} scrollEventThrottle={16}>
        <View style={styles.content}>
          <Ionicons name="repeat-outline" size={48} color={theme.colors.textMuted} />
          <Text style={styles.message}>{t('admin.plans.comingSoon', 'Subscription plans management coming soon')}</Text>
        </View>
