...
export default function AdminOperationalReportScreen() {
  const { t } = useTranslation();
  const { handleScroll, headerStyle, headerOpacity } = useScrollHeader();
  return (
    <View style={styles.root}>
      <View style={[styles.header, headerStyle]}>
        <Text style={[styles.title, { opacity: headerOpacity }]}>{t('admin.reports.operational', 'Operational Reports')}</Text>
      </View>
      <ScrollView style={styles.scroll} onScroll={handleScroll} scrollEventThrottle={16}>
        <View style={styles.content}>
          ...
          <Text style={styles.message}>{t('admin.reports.comingSoon', 'Operational reports coming soon')}</Text>
        </View>
