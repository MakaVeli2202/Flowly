...
namespace Glanz.API.Services
{
    ...
    public class AdminNotificationService : IAdminNotificationService
    {
        ...
        public Task NotifyJobCompletedAsync(Booking booking)
        {
            ...
            var customerMessage = $"All done{durationText}! Your car is looking its best. Thank you for choosing Glanz — we hope to see you again soon!";
            return NotifyAdminsAndCustomerAsync(
        ...
        private async Task CreateForAllAdminsAsync(NotificationType type, string message, int? bookingId = null)
        {
            ...
            await _expoPush.SendBatchAsync(expoTokens, "Glanz", message, new { type = type.ToString(), bookingId });
        }
        ...
        private async Task CreateForUserAsync(int userId, NotificationType type, string message, int? bookingId = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(user.ExpoPushToken))
                await _expoPush.SendAsync(user.ExpoPushToken, "Glanz", message, new { type = type.ToString(), bookingId });
        }
