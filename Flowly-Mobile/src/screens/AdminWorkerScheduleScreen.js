...
export default function AdminWorkerScheduleScreen() {
  const { t } = useTranslation();
  const { handleScroll, headerStyle, headerOpacity } = useScrollHeader();
  return (
    <View style={styles.root}>
      <View style={[styles.header, headerStyle]}>
        <Text style={[styles.title, { opacity: headerOpacity }]}>{t('admin.schedule.title', 'Worker Schedule')}</Text>
      </View>
      <ScrollView style={styles.scroll} onScroll={handleScroll} scrollEventThrottle={16}>
        <View style={styles.content}>
          ...
          <Text style={styles.message}>{t('admin.schedule.comingSoon', 'Worker schedule management coming soon')}</Text>
        </View>
